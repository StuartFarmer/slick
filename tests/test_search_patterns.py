...
def test_self_consistency_counts_answers_without_a_judge():
    ...
    sampler = module.SolutionSampler("Capital of France?", provider)
    ...
    assert all("answer" in text and "properties" in text for text in provider.inputs)


def test_sampler_rejects_blank_answer():
    module = importlib.import_module("examples.self_consistency")
    sampler = module.SolutionSampler("Question", FakeProvider([{"answer": "  ", "approach": "a"}]))
...
def test_tree_search_prunes_and_expands_partial_state():
    ...
    search = module.ThoughtSearch("Task", provider, max_depth=2, beam_width=1, breadth=2)
    result = asyncio.run(search.run())
    assert result.answer == "done"
    assert result.steps == ("promising", "finish")
    assert search.frontier == [result]
    assert "promising" in provider.inputs[3]
    assert "discard me" not in provider.inputs[3]
    assert "properties" in provider.inputs[0]


def test_tree_depth_exhaustion_and_nonfinite_scores():
    module = importlib.import_module("examples.tree_of_thoughts")
    provider = FakeProvider([{"candidates": [{"step": "partial"}]}, {"score": 0.5}])
    search = module.ThoughtSearch("Task", provider, max_depth=1, beam_width=1, breadth=1)
    result = asyncio.run(search.run())
...
def test_tree_rejects_excessive_expansion():
    module = importlib.import_module("examples.tree_of_thoughts")
    provider = FakeProvider([{"candidates": [{"step": "a"}, {"step": "b"}]}])
    search = module.ThoughtSearch("Task", provider, breadth=1)
...
def test_least_to_most_uses_decomposition_and_prior_answers():
    ...
    solver = module.LeastToMost("Find total cost", provider, max_subproblems=2)
    assert asyncio.run(solver.run()) == "$12"
    assert len(solver.solutions) == 3
    assert "Find unit cost" in provider.inputs[1]
    assert "$3" in provider.inputs[2]
    assert "$3" in provider.inputs[3] and "4" in provider.inputs[3]
    assert "Find total cost" in provider.inputs[3]
    assert len(provider.inputs) == 4


def test_least_to_most_rejects_plan_over_bound():
    module = importlib.import_module("examples.least_to_most")
    solver = module.LeastToMost(
...
def test_cli_runs_demo_and_rejects_invalid_limits(name):
    result = subprocess.run(
        [sys.executable, "-m", f"examples.{name}", "--provider", "demo"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        timeout=10,
...
def test_tree_constructor_validates_all_bounds(limit, value):
    ...
    with pytest.raises(ValueError, match=limit):
        module.ThoughtSearch("Task", FakeProvider([]), **{limit: value})
...
def test_sampler_normalizes_whitespace_and_breaks_ties_by_first_sample():
    ...
    sampler = module.SolutionSampler("Task", provider)
...
def test_tree_dead_end_stops_without_more_calls():
    ...
    search = module.ThoughtSearch("Task", provider, max_depth=5)
